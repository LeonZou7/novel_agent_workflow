...
2. **自洽性检查**
   - 检查世界观逻辑是否闭环，无漏洞
   - 检查力量体系是否自洽（等级划分、获取方式、限制规则）
   - 检查地理、历史、势力之间是否逻辑一致
   - 如发现不自洽，补充缺失的逻辑环节或调整不合理设定

3. **约束合规检查**
   - 检查生成内容是否符合 `.novel/config.yml` 中的 constraints
   - 如发现违规，调整为符合约束的虚构替代方案

4. **KG 一致性检查**
   - 检查生成内容与已有 KG 条目的一致性
   - 检查各维度（overview、power_system、geography、history、factions）之间是否一致
   - 如发现冲突，修正为一致
