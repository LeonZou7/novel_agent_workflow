...
args:
  - name: command
    description: 子命令 (concept / start / continue / status / work-queue / backtrack)
    required: false
---
...
1. `.novel/config.yml` — 项目配置（含 constraints 硬规则段）
...
读取 config.yml 中的 `constraints` 段。如 `enabled: true`，在调度每个生成类子 Agent（outline、world、character、draft）时，将约束规则作为上下文的一部分传入，确保子 Agent 从源头遵守约束。
...
| 用户命令 | 行为 |
|---------|------|
| `/znovel init` | 直接执行 init_project.py |
| `/znovel concept` | 触发核心创意脑暴 |
| `/znovel start` | 从核心创意开始全流程 |
| `/znovel start [stage]` | 从指定 stage 开始调度 |
| `/znovel continue` | 从 current_stage 继续 |
| `/znovel status` | 读取 state.yml + work_queue.yml + brainstorm/，展示状态 |
...
| `/znovel backtrack <stage>` | 回溯处理 |
| `/znovel work-queue` | 展示工作队列 |

## 核心创意脑暴流程 `/znovel concept`

触发条件：用户输入 `/znovel concept` 或 `/znovel start`
...
| 跳过核心创意直接调子 Agent | 提示「核心创意尚未确定，建议先完成 /znovel concept。是否跳过？」 |
